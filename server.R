server <- function(input, output, session) {
  ...
  observeEvent(input$browse_users_data, {
    ...
    if (grepl("study_objects", path)) {
      showModal(
        modalDialog(
          div(
            paste0("Do you want to clone or download ", relative_path(), "?"),
            style = "font-size: 25px; margin-bottom: 20px; text-align: center;"
          ),
          footer = tagList(
            actionButton("confirm_clone_super_user", "Clone", class = "btn-primary"),
  ...
  output$download_super_user <- downloadHandler(
    filename = function() {
      rp <- relative_path()
      safe_name <- gsub("/", "_", rp)
      paste0(tools::file_path_sans_ext(safe_name), ".json")
    },
    content = function(file) {
